...
# импорт данных
df1 <- read.csv("C:/Users/artus/OneDrive/Документы/ЛР8_Data.csv", sep = "," , header = TRUE)
years=1989:2018
View(df1)
...
plot(
  ...
  VVP_growth,
  xlab='Год',
  ylab='Прирост ВВП в USD',
  main='Прирост ВВП в USD',
  type='b',
  lty=1,
  pch=1, 
  lwd=2
...
# представление в виде эллипосов
library(ellipse)
plotcorr(cor(df6))
...
# независимая переменная - расходы на медицину,зависимая – значение продолжительности жизни.
fit <- lm(Average_life ~ Changes_medical_cost, 
          data=states)
...
par(mfrow=c(1, 2))
plot(states$`Changes_medical_cost`, states$`Average_life`, xaxt="n", xlab="Расходы на медицину", ylab="Продолжительность жизни", col="blue", main="График реальных значений продолжительности жизни")
abline(fit, col="red")
plot(pred, xaxt="n", xlab="Расходы на медицину", ylab="Продолжительность жизни", col="blue", main="График прогнозируемых значений продолжительности жизни")
library(ellipse)
